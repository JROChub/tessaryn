...
fn build_proof_bundle(
    ...
    let mut rootprint = Rootprint::new("world-cell", artifact).map_err(|error| error.to_string())?;
    let root_id = RootprintId::new(rootprint.root_branch.clone()).map_err(|error| error.to_string())?;
    ...
    let mut memory_capsule = MemoryCapsuleBuilder::new(format!(
        "world_cell_{}",
        &evidence.canonical_digest[..16]
    ))
...
fn create_seal(canonical_cell: &[u8], evidence_json: &[u8], seed: &[u8]) -> Result<Seal, String> {
    ...
    verify_seal(canonical_cell, evidence_json, &serde_json::to_vec(&seal).map_err(|error| error.to_string())?)?;
    Ok(seal)
}

fn verify_seal(canonical_cell: &[u8], evidence_json: &[u8], seal_json: &[u8]) -> Result<(), String> {
    let evidence: Evidence = serde_json::from_slice(evidence_json)
        .map_err(|error| format!("invalid assurance evidence: {error}"))?;
    let seal: Seal = serde_json::from_slice(seal_json)
        .map_err(|error| format!("invalid browser seal: {error}"))?;
    if seal.schema != SEAL_SCHEMA || !seal.verified || seal.provider != PROVIDER ||
    ...
    seal.proof_bundle.pha.verify().map_err(|error| error.to_string())?;
    seal.proof_bundle
        .rootprint
        .verify()
        .map_err(|error| error.to_string())?;
    let replay = seal.proof_bundle
        .rootprint
        .replay()
        .map_err(|error| error.to_string())?;
    let memory_report = seal.proof_bundle
        .memory_capsule
        .verify(MemoryVerificationPolicy::strict())
        .map_err(|error| error.to_string())?;
    ...
    let capsule_digest = seal.proof_bundle
        .memory_capsule
        .calculate_capsule_digest()
        .map_err(|error| error.to_string())?;
    ...
    if inputs["canonical_digest"].as_str() != Some(evidence.canonical_digest.as_str()) ||
        ...
        return Err("Power House PHA public inputs do not bind the World Cell evidence".to_string());
    }
    if !seal.assurance_record.contains(&format!("profile={PROFILE}\n")) ||
        !seal.assurance_record.contains(&format!("digest={}\n", hex::encode(envelope))) ||
        !seal.assurance_record.contains(&format!("signature={}\n", seal.signature_base64)) {
