...
bool stronger_hypothesis(const MotionHypothesis& candidate,const MotionHypothesis& incumbent,std::size_t matches){const std::size_t material=std::max<std::size_t>(3U,matches/20U);if(candidate.inliers>=incumbent.inliers+material)return true;if(incumbent.inliers>=candidate.inliers+material)return false;if(std::isfinite(candidate.median_residual)&&std::isfinite(incumbent.median_residual)){if(candidate.median_residual<incumbent.median_residual*0.8F)return true;if(incumbent.median_residual<candidate.median_residual*0.8F)return false;}if(candidate.positive_depth!=incumbent.positive_depth)return candidate.positive_depth>incumbent.positive_depth;if(candidate.translation.observability>incumbent.translation.observability+0.01F)return true;if(incumbent.translation.observability>candidate.translation.observability+0.01F)return false;const float candidate_cost=candidate.median_residual+candidate.rotation_degrees*2.0e-5F;const float incumbent_cost=incumbent.median_residual+incumbent.rotation_degrees*2.0e-5F;return candidate_cost<incumbent_cost;}
