...
float epipolar_model_score(const std::vector<Pair>& correspondences,const Mat3& rotation){std::vector<float> weights(correspondences.size(),1.0F);const auto translation=estimate_translation(correspondences,weights,rotation);if(!finite(translation.direction)||length(translation.direction)<0.9F)return std::numeric_limits<float>::infinity();std::vector<float> residuals;residuals.reserve(correspondences.size());for(const auto& pair:correspondences){if(pair.current.match_error>2.5F)continue;residuals.push_back(epipolar_residual(pair,rotation,translation.direction));}if(residuals.size()<10U)return std::numeric_limits<float>::infinity();const float center=median(residuals);std::sort(residuals.begin(),residuals.end());const float upper=residuals[(residuals.size()*3U)/4U];return center+0.35F*upper+rotation_angle_degrees(rotation)*1.5e-4F;}
