...
Mat3 axis_rotation(int axis,float angle){const float c=std::cos(angle),s=std::sin(angle);if(axis==0)return {1,0,0,0,c,-s,0,s,c};if(axis==1)return {c,0,s,0,1,0,-s,0,c};return {c,-s,0,s,c,0,0,0,1};}
float epipolar_model_score(const std::vector<Pair>& correspondences,const Mat3& rotation){std::vector<float> weights(correspondences.size(),1.0F);const auto translation=estimate_translation(correspondences,weights,rotation);if(!finite(translation.direction)||length(translation.direction)<0.9F)return std::numeric_limits<float>::infinity();std::vector<float> residuals;residuals.reserve(correspondences.size());for(const auto& pair:correspondences){if(pair.current.match_error>2.5F)continue;residuals.push_back(epipolar_residual(pair,rotation,translation.direction));}if(residuals.size()<10U)return std::numeric_limits<float>::infinity();const float center=median(residuals);std::sort(residuals.begin(),residuals.end());const float upper=residuals[(residuals.size()*3U)/4U];return center+0.35F*upper+rotation_angle_degrees(rotation)*1.0e-6F;}
Mat3 refine_epipolar_rotation(const std::vector<Pair>& correspondences){std::vector<float> weights(correspondences.size(),1.0F);Mat3 best=identity3();float best_score=epipolar_model_score(correspondences,best);const Mat3 direct=estimate_rotation(correspondences,weights);const float direct_score=epipolar_model_score(correspondences,direct);if(direct_score<best_score){best=direct;best_score=direct_score;}const float steps_degrees[]={4.0F,2.0F,1.0F,0.5F,0.25F};for(float step_degrees:steps_degrees){const float step=step_degrees/kRadToDeg;bool improved=true;for(int sweep=0;sweep<3&&improved;++sweep){improved=false;Mat3 selected=best;float selected_score=best_score;for(int axis=0;axis<3;++axis){for(float sign:{-1.0F,1.0F}){const Mat3 candidate=multiply(axis_rotation(axis,sign*step),best);const float score=epipolar_model_score(correspondences,candidate);if(score+1.0e-7F<selected_score){selected=candidate;selected_score=score;improved=true;}}}best=selected;best_score=selected_score;}}return best;}
