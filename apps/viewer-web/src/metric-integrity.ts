...
/**
 * Fail-closed admission policy for v0.22.
 *
 * This module deliberately does not estimate pose or depth. It decides whether
 * evidence produced by a pose/depth backend is strong enough to enter the
 * sovereign reconstruction. Renderers must not bypass this decision.
 */
export function evaluateIntegrity(input: IntegrityObservation): IntegrityDecision {
  ...
  const inlierRatio = input.correspondenceCount > 0
    ? input.inlierCount / input.correspondenceCount
    : 0;
  ...
  const poseSolved = input.poseAuthority !== "none" &&
    enoughFeatures && enoughCorrespondences && enoughInliers && boundedError && boundedTrackingCovariance;
  ...
  const metricScale = Boolean(input.metricScaleSource) && (metricPose || metricDepth) &&
    finite(input.baselineMeters ?? Number.NaN) && (input.baselineMeters ?? 0) > 0;
  ...
  const maySealMetricCell = mayConfirmGeometry && metricScale &&
    input.confirmedSurfels >= 512 && input.uncertainSurfels <= input.confirmedSurfels * 3;
...
export function assertMetricSeal(decision: IntegrityDecision): void {
  if (!decision.maySealMetricCell) {
    throw new Error(`Metric World Cell seal rejected: ${decision.reasons.join("; ")}`);
  }
}
