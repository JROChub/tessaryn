...
function stage(root: HTMLElement): AuthorityStage {
  ...
  return value === "tracking" || value === "moment-ready" || value === "seal-ready" ? value : "forming";
}
...
function installStyle(): void {
  ...
  style.textContent = `
    ...
    .reality-card{position:absolute;left:14px;bottom:16px;width:min(438px,calc(100% - 28px));padding:13px 14px;background:linear-gradient(180deg,rgba(4,9,16,.84),rgba(2,5,9,.72));border:1px solid rgba(98,216,255,.22);border-radius:8px;box-shadow:0 18px 60px rgba(0,0,0,.38);backdrop-filter:blur(18px)}
    .reality-card header{display:flex;align-items:center;justify-content:space-between;gap:14px;margin:0 0 10px}.reality-card small{display:block;font:700 9px/1.2 ui-monospace,monospace;letter-spacing:.15em;color:#7a8da1;text-transform:uppercase}.reality-card b{display:block;margin-top:4px;font:800 13px/1.12 ui-monospace,monospace;letter-spacing:.08em;color:#f2fcff;text-transform:uppercase}
    .reality-score{min-width:72px;text-align:right;color:#8ce9ff;font:800 18px/1 ui-monospace,monospace}.reality-meter{height:6px;background:rgba(96,118,143,.18);border-radius:999px;overflow:hidden;box-shadow:inset 0 0 0 1px rgba(255,255,255,.05)}.reality-meter i{display:block;width:0;height:100%;background:linear-gradient(90deg,#c76650,#d7bb68,#7de6ff);transition:width .18s ease}
    .reality-axis{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:9px 0}.reality-axis span{padding:8px;background:rgba(7,15,24,.62);border:1px solid rgba(255,255,255,.06);border-radius:5px}.reality-axis em{display:block;margin-bottom:6px;color:#72879c;font:700 8px/1 ui-monospace,monospace;letter-spacing:.13em;text-transform:uppercase;font-style:normal}.reality-axis i{display:block;height:4px;background:rgba(83,102,120,.2);border-radius:999px;overflow:hidden}.reality-axis i::before{content:"";display:block;width:var(--value,0%);height:100%;background:linear-gradient(90deg,#63d5ff,#d8c06c);transition:width .18s ease}
    .reality-instruction{margin:0;color:#cbe1ef;font:600 12px/1.45 ui-sans-serif,system-ui,sans-serif}.reality-instruction strong{display:block;color:#fff;font:800 12px/1.25 ui-monospace,monospace;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px}.reality-ledger{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:6px;margin-top:9px;color:#9ab0c2;font:700 9px/1.2 ui-monospace,monospace;letter-spacing:.08em;text-transform:uppercase}.reality-ledger span{padding:7px 8px;border-radius:4px;background:rgba(6,12,19,.56);border:1px solid rgba(255,255,255,.05)}.reality-ledger b{display:block;margin-top:3px;color:#e6f9ff;font-size:10px;letter-spacing:0;text-transform:none}
    .reality-gates{position:absolute;right:16px;bottom:18px;display:grid;gap:8px;width:160px}.reality-gate{padding:9px 10px;border-radius:7px;background:rgba(3,8,13,.74);border:1px solid rgba(255,255,255,.08);backdrop-filter:blur(14px);text-align:center;box-shadow:0 14px 40px rgba(0,0,0,.26)}.reality-gate small{display:block;color:#70869a;font:800 8px/1 ui-monospace,monospace;letter-spacing:.14em;text-transform:uppercase}.reality-gate b{display:block;margin-top:5px;color:#d5e5ef;font:900 13px/1 ui-monospace,monospace;letter-spacing:.1em;text-transform:uppercase}.reality-gate[data-open="true"]{border-color:rgba(115,238,255,.45);box-shadow:0 0 30px rgba(98,216,255,.18),inset 0 0 0 1px rgba(115,238,255,.16)}.reality-gate[data-open="true"] b{color:#8df2ff}
    @media(max-width:760px){.reality-card{left:8px;right:8px;bottom:10px;width:auto}.reality-gates{right:10px;top:80px;bottom:auto;width:124px}.reality-axis{grid-template-columns:1fr}.reality-ledger{grid-template-columns:1fr}}
  `;
...
function build(panel: HTMLElement): Cockpit {
  ...
  shell.innerHTML = `
    ...
    <section class="reality-card" aria-live="polite">
      ...
      <div class="reality-axis"><span><em>MOTION</em><i data-role="motion"></i></span><span><em>EVIDENCE</em><i data-role="evidence"></i></span><span><em>LINEAGE</em><i data-role="lineage"></i></span></div>
    ...
    <div class="reality-gates"><span class="reality-gate" data-role="moment" data-open="false"><small>Moment</small><b>LOCKED</b></span><span class="reality-gate" data-role="seal" data-open="false"><small>Seal</small><b>LOCKED</b></span></div>
  `;
  panel.append(shell);
  return {
    shell,
...
function instruction(mask: number, tracked: boolean): { title: string; detail: string } {
  if (mask & REJECT_RECEIPT) return { title: "AUTHORITY SYNCING", detail: "The visual field stays live while Keyxym rebuilds a valid receipt chain. Keep scanning slowly; no Moment or seal opens until native receipts validate." };
  if (mask & REJECT_REPROJECTION) return { title: "REDUCE MOTION BLUR", detail: "Move more slowly, keep textured edges sharp, and maintain the same objects in view until reprojection returns below the authority limit." };
  if (mask & (REJECT_POSE | REJECT_TRACKING | REJECT_PARALLAX | REJECT_DEGENERATE)) return { title: tracked ? "REACQUIRE TRACKING" : "CREATE 3D PARALLAX", detail: "Move slowly sideways around textured objects at different depths. Keep them in view; avoid flat walls, digital screens, blur, and repeating patterns." };
  if (mask & (REJECT_GEOMETRY | REJECT_CONTINUITY)) return { title: "BUILD VERIFIED GEOMETRY", detail: "Continue a slow arc, then revisit the same surfaces so Keyxym can confirm them across frames before enabling a Moment." };
  if (mask & REJECT_SCALE) return { title: "METRIC SCALE UNBOUND", detail: "Relative World Cells can form now. Metric authority requires a verified depth, event, or calibration adapter receipt before scale can be sealed." };
  return { title: "AUTHORITY FIELD STABLE", detail: "Keep the same textured geometry in view. When the native gate opens, commit the Moment before changing viewpoint." };
}
...
function render(cockpit: Cockpit, state: Snapshot, tracked: boolean): void {
  ...
  cockpit.secondary.textContent = state.momentGate ? "The native v0.26 gate currently allows a Moment. Commit now, then keep scanning until Seal opens." : next.detail;
...
function draw(canvas: HTMLCanvasElement, state: Snapshot, time: number): void {
  ...
  for (let radius = 72; radius < Math.max(w, h) * 0.65; radius += 74) {
    context.beginPath(); context.arc(cx, cy, radius + Math.sin(pulse + radius * 0.01) * 5 * authority, 0, Math.PI * 2); context.stroke();
  }
  ...
  for (let index = 0; index < rays; index += 1) {
    ...
    context.quadraticCurveTo(cx + Math.cos(angle + Math.PI / 2) * bend, cy + Math.sin(angle + Math.PI / 2) * bend, cx + Math.cos(angle) * radius, cy + Math.sin(angle) * radius * 0.62);
    context.stroke();
  }
  context.globalAlpha = 0.80;
  context.lineWidth = 2;
  context.strokeStyle = state.sealGate ? "rgba(154,255,232,.85)" : state.momentGate ? "rgba(112,229,255,.78)" : "rgba(211,180,101,.66)";
  context.beginPath(); context.arc(cx, cy, 52 + authority * 36, -Math.PI / 2, -Math.PI / 2 + Math.PI * 2 * Math.max(0.04, authority)); context.stroke();
  context.restore();
}
...
export function installWorldCellGuidance(): () => void {
  ...
  rootObserver.observe(root, {
    attributes: true,
    attributeFilter: ["data-keyxym-authority", "data-authority-stage", "data-authority-rejection-mask", "data-forming-samples", "data-authoritative-surfels", "data-geometry-revision", "data-moment-allowed", "data-seal-allowed"],
  });
  const observer = new MutationObserver(update);
  const watched = [
    capture,
    ...["frame-count", "tracking-value", "parallax-value", "error-value", "coverage-value", "confirmed-value", "uncertain-value", "rejected-value", "quality-meter"]
      .map((id) => document.getElementById(id))
      .filter((node): node is HTMLElement => node instanceof HTMLElement),
  ];
  for (const node of watched) observer.observe(node, { childList: true, characterData: true, attributes: true, subtree: true });
  update();
  animation = requestAnimationFrame(tick);
  return () => { cancelAnimationFrame(animation); rootObserver.disconnect(); observer.disconnect(); cockpit.shell.remove(); };
}
