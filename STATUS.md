...
| Chronofold | Experimental | Three synthetic Moments and two conflict branches |
| Inside-out Trace | Experimental | Evidence, lineage, and removable meaning views |
| Real device capture adapter | Not implemented | No camera or sensor is represented by the synthetic fixture |
| Real 20-40 meter site | Not implemented | No captured dataset is represented as real |
| Three real capture sessions | Not implemented | Synthetic dates are presentation fixtures only |
| Private multi-device Weave | Speculative | Architecture only |
...
TESSARYN v0.1 is not reached until a real bounded place is captured in three
sessions, privacy-reviewed, independently reconstructed, verified offline,
measured against the reference budgets, and reproduced on a second machine.
The synthetic Origin is deliberately labeled `experimental-synthetic` in both
the fixture and interface.
